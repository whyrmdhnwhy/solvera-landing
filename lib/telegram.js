...
async function handleScanFree(chatId, address) {
  ...
  try {
    ...
    if (risk?.flags?.some(f => f.label === "PROGRAM ACCOUNT")) {
      return sendTG(chatId, "🔷 PROGRAM ACCOUNT\n\nThis is a Solana smart contract, not a user wallet. Risk signals are designed for wallets that hold and move funds, not programs that process transactions for everyone.");
    }
...
async function handlePaidUnlock(chatId, address) {
  ...
  try {
    ...
    if (risk.typologies) {
      const triggered = Object.values(risk.typologies).filter(t => t.triggered);
      if (triggered.length > 0) {
        for (const t of triggered) {
          reply += `🔍 ${t.code}: ${t.name} (+${t.score})\n`;
          if (t.detail) {
            reply += `${t.detail}\n\n`;
          } else {
            reply += `Pattern matches high risk behavior.\n\n`;
          }
        }
      } else {
        reply += `None notable.\n\n`;
