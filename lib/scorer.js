  ...
  for (const tx of ctx.transactions) {
    ...
    if (Array.isArray(tx.instructions)) {
      for (const ix of tx.instructions) {
        ...
        if (Array.isArray(ix.accounts)) ix.accounts.forEach(a => addrs.add(a))
        if (Array.isArray(ix.innerInstructions)) {
          for (const inner of ix.innerInstructions) {
            if (inner.programId) addrs.add(inner.programId)
            if (Array.isArray(inner.accounts)) inner.accounts.forEach(a => addrs.add(a))
          }
        }
      }
    }
...
function detectT4(ctx, counterparties, t2, t3, t5) {
  ...
  // Risk Override: Drainers always launder via bridges
  const selfHits = checkCounterparties([ctx.address])
  const isDrainer = selfHits.mixers.some(m => m.category === 'exploit_drainer')
  ...
  const bridgeFound     = hits.bridges.length > 0 || protocolBridges.length > 0 || isDrainer

  if (!bridgeFound) return result

  const firstBridge    = isDrainer ? { label: 'Assumed Bridge Abuse (Known Exploit Drainer)' } : (hits.bridges[0] || { label: protocolBridges[0]?.program || 'known bridge program' })
  const hasAmplifier   = t2.triggered || t3.triggered || t5.triggered || isDrainer
